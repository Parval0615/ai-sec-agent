...
### 苏格拉底问答：Ed25519 签名 — 为什么 Hash 不够还需要签名？

围绕 Phase B.1 Tool Integrity Verification 的讨论。

**1. 为什么 SHA256 hash 不够，还需要 Ed25519 签名？**

Hash 验证完整性（"文件有没有被改"），签名验证来源（"是谁签的"）。如果只有 hash，攻击者修改文件后重算 hash 放进 manifest，验证照样通过。Ed25519 签名需要私钥——攻击者没有私钥就生成不了有效签名。hash + 签名 = 完整性 + 真实性。

**2. 公钥被替换了怎么办？**

和审计日志哈希链"全链替换"同源——本地验证依赖公钥的真实性。解决方案都是外部锚定：硬编码公钥在启动代码中，或从只读证书链获取。

**3. manifest 的 permission_scope 可靠吗？**

签名保证 manifest 未被篡改——不保证开发者写的 permission_scope 准确。如果开发者在 manifest 写 `read only` 但工具实际实现 `DROP`，签名不会发现。这是"信任根"问题：签名证明"谁签的"，不证明"签的内容是诚实的"。

### 苏格拉底问答：Indirect Prompt Injection — 为什么防火墙守不了文档攻击？

围绕 Phase A 完整攻击链的讨论。

**1. Direct vs Indirect Prompt Injection 的本质区别是什么？**

防火墙（3.1）检查的是用户输入——它的工作前提是"攻击载荷在对话输入中"。Indirect Injection 中，攻击载荷在文档/网页/邮件里——用户输入是"总结这个文档"，完全无害。防火墙看不到攻击，但 LLM 仍然会被污染。

这就是为什么纵深防御需要三层：入库扫描（L1）检测文档中的隐藏指令、策略引擎（L3）在工具执行前截杀危险参数、审计日志记录完整路径。

**2. 为什么 HTML 比 PDF 更容易注入？**

CSS `display:none` 是前端开发的日常模式——任何网页都可能有。PDF 的白字攻击需要攻击者特意构造渲染属性（字号、颜色），而 HTML 的隐藏文本是内容本身的一部分。所以 HTML L1 扫描的误报率天然更高——Rule 8 对 `display:none` 只能标记不直接拦截，需要 L2 语义分析确认。

**3. 多跳间接注入为什么难以防御？**

单 chunk 扫描看不到跨文档的攻击链——文档A说"查文档B"，文档B含载荷，每步单独看都正常。这本质上是信息流追踪问题——需要知道"Agent 为什么搜索了这个文档"才能判断安全性。当前架构不支持这个级别的上下文感知。

**4. Indirect Injection 的核心防御思路是什么？**

不是"100% 拦截"（不可能——语义合法的载荷总是存在），而是"纵深防御"：L1 规则拦明显模式 → L2 LLM 分类拦语义模糊 → 策略引擎在工具执行前拦危险参数 → 审计日志确保事后可追溯。每层的漏过率相乘才是最终风险。
